...
fig_topic_distribution = px.bar(
    ...
    height = 500   # It sets the height of the graph

)

fig_topic_distribution.update_layout(
    plot_bgcolor = "#f9f9f9",      # Name of background colors taken from website is added in the AI documentation file
...
keyword_df = df[["Topic_Label", "topic_1", "topic_2", "topic_3", "topic_4"]].melt(     # ChatGPT conversation 1 (AI Documentation)
...
fig_average_count = px.bar(
    ...
    text_auto = True,
    height = 600,
    color = "Average_Word_Count",
    
)

fig_average_count.update_layout(
    xaxis_tickangle=45,
    plot_bgcolor="#f0f8ff",
    paper_bgcolor="#f0f0f0",
    font=dict(size=13),
